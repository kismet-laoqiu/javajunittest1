...
public class QrCodeUtils {
     ...
     * @throws Exception
     */
    public static void encode(String content,String codenum, String logoPath, OutputStream output, boolean needCompress)
            throws Exception {
        BufferedImage image = QrCodeUtils.createImage(content, logoPath, needCompress);
        byte[] date = ImageUtil.imageToBytes(image,"png");
        CodeClient codeClient = new CodeClient();
        String s = codeClient.queryCodeAssetAmount(codenum);
        Base64.Encoder encoder = Base64.getEncoder();
        Base64.Decoder decoder = Base64.getDecoder();
        byte[] date2 = decoder.decode(s);
        if (Arrays.equals(date,date2))
            ImageIO.write(image, FORMAT, output);
    }

    public static byte[] encodes(String content, String logoPath, boolean needCompress) throws Exception {
        BufferedImage image = QrCodeUtils.createImage(content, logoPath, needCompress);
